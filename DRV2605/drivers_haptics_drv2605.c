...
static int drv2605_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C))
	{
		...
		printk(KERN_INFO "BuddyDRV[%s]I2C check functions failed\n",
			__FUNCTION__);
		return -ENODEV;
	}
	...
	printk(KERN_INFO "BuddyDRV[%s]Succeed to allocate memory\n",__FUNCTION__);
	...
	printk(KERN_INFO "BuddyDRV[%s]Set gpio\n",__FUNCTION__);
	if(pDrv2605data->PlatData.GpioEnable){
		...
		if(err < 0){
			...
			printk(KERN_INFO "BuddyDRV[%s]GPIO request enable error\n",
				__FUNCTION__);
			goto exit_gpio_request_failed;
		}
	...
	printk(KERN_INFO "BuddyDRV[%s]Enable power to the chip\n",__FUNCTION__);
	err = drv2605_reg_read(pDrv2605data, STATUS_REG);
	if(err < 0){
		printk("%s, i2c bus fail (%d)\n", __FUNCTION__, err);

		printk(KERN_INFO "BuddyDRV[%s]i2c bus fail\n",__FUNCTION__);
		goto exit_gpio_request_failed;
	}else{
		printk("%s, i2c status (0x%x)\n", __FUNCTION__, err);

		printk(KERN_INFO "BuddyDRV[%s]i2c status\n",__FUNCTION__);
		status = err;
	}
	/* Read device ID */
	pDrv2605data->device_id = (status & DEV_ID_MASK);

	printk(KERN_INFO "BuddyDRV[%s]Device id %p\n",__FUNCTION__,
			(void *)pDrv2605data->device_id);
    ...
    printk(KERN_INFO "BuddyDRV[%s]Change mode data\n",__FUNCTION__);
    Haptics_init(pDrv2605data);
	
	pDRV2605data = pDrv2605data;
    printk("drv2605 probe succeeded\n");
    printk(KERN_INFO "BuddyDRV[%s]drv2605 probe successed\n",__FUNCTION__);
    return 0;
